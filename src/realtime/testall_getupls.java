...
public class testall_getupls{
	...
	public static void main(String[]args){
		...
		ArrayList<dukas> arr = parseDukas.parseOne("C:\\Users\\Acer\\Documents\\forex3\\","USDSGD_UTC_1 Min_Bid_2008.01.01_2015.03.12.csv") ;
		...
		NumberFormat formatter2 = new DecimalFormat("#0000");
			...
			for(int i=0;i<50;i++){
				...
				for(double tp=0.0001;tp<0.01;tp+=.0002){ scount++ ; alist.add(new ArrayList<Double>()) ;
					int tpcount = 0 ; 
					for(double stop=0.0005;stop<0.020;stop+=0.0005){ tpcount++ ; 
						int icount = 0 ; 
						
				//LinePlot lp = new LinePlot() ; 
				int index = (int)(arr.size()*Math.random()*.5) ;			
				testall_getupls p2 = new testall_getupls() ; 
				p2.tps[0] = tp ; 
				p2.stops[0] = -stop ; 			
			...
			//SaveArray.saveArrayList(alist, "money_talks\\"+formatter2.format(scount)+"_"+formatter2.format(tpcount)+"_"+formatter.format(stop)+"_"+formatter.format(tp)+".txt");
				...
				SaveArray.saveArrayList(alist, "mts\\mt_USDSGD\\" + formatter2.format(i) +".txt");
			}
	}
}
