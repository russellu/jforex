...
public class RobinHood{
	...
	public RobinHood(String pair, double tp, double stop, int mvg1, int mvg2, int mvg3, double slope){ // where pers is a 1x3 array of mvg periods
		initSpreads(pair) ; 
	...
	public void calculateNext(ArrayList<ArrayList<Double>>mvgs,double price,ArrayList<Integer> periods){
		for(int i=0;i<mvgs.size();i++){
			...
			if(mvg.size()<period){
				...
				//mvg.add(mean(raw.subList(raw.size()-period, raw.size()))) ;
				mvg.add(mvg.get(mvg.size()-1) - raw.get(raw.size()-period-1)/period + raw.get(raw.size()-1)/period) ; 
				//double mvg1 = mean(raw.subList(raw.size()-period, raw.size())) ; 
				//double mvg2 = mvg.get(mvg.size()-1) - raw.get(raw.size()-period-1)/period + raw.get(raw.size()-1)/period ;
				//mvg.add(mvg1) ; 
				//System.out.println("m1 = " + mvg1 + " m2 = " + mvg2 + " i = " + i + " diff = " + (mvg1-mvg2) + " sublist size = " + raw.subList(raw.size()-period, raw.size()).size()) ; 
				//System.out.println("mvg1 = " + mperiods[0] + " mvg2 = " + mperiods[1] + " mvg 3 = " + mperiods[2]) ;
			}
		}
	}
