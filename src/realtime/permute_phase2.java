	...
	public static void main(String[]args){
		...
		NumberFormat formatter2 = new DecimalFormat("#0000");
			...
			for(int i=0;i<100;i++){
				...
				for(double tp=0.5;tp<80;tp+=3){ scount++ ; alist.add(new ArrayList<Double>()) ;
					int tpcount = 0 ; 
					for(double stop =0.5;stop<250;stop+=10){ tpcount++ ; 
						int icount = 0 ; 
						
				//LinePlot lp = new LinePlot() ; 
				int index = (int)(arr.size()*Math.random()*.5) ;			
				permute_phase2 p2 = new permute_phase2() ; 
				p2.tps[0] = tp ; 
				p2.stops[0] = -stop ; 			
				for(int j=index;j<index+24*60*30;j++){
