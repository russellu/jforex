...
public class permute_trailstop {
	...
	double[] trails = new double[1] ;
	double[] trailUPLs = new double[1] ; 
	double trailStop = 0.0010 ; 
	boolean trailStopSet = false ; 
	
	
	
	public permute_trailstop(){
		int[] pers = {12,72,122} ; 
	...
	public void checkTrades(){
		...
		if(currentDirection==0 )return ; //if no trade in progress, return
			...
			if(upls[0] > tps[0] || upls[0] < stops[0]){
				if(!trailStopSet && upls[0] > tps[0]){
					//System.out.println("Setting trailing stop ") ; 
					trailStopSet = true ;
					trails[0] = raw.get(raw.size()-1) - trailStop ; 
					trailUPLs[0] = upls[0] ; 
				}				
				else if (trailStopSet){
					//System.out.println("trailstop set, not exited...") ; 
					if(upls[0] > trailUPLs[0]){
					//	System.out.println("updating trailing stop ") ; 

						trails[0] = raw.get(raw.size()-1) - trailStop ; // shift trailing stop
						trailUPLs[0] = upls[0] ; 
					}
					else if((raw.get(raw.size()-1)) < trails[0]){
				//		System.out.println("exiting trailing stop") ;
						close = true ; 
					}
				}
				else close = true ; //hit stop
	...
	public static void main(String[]args){
		...
		ArrayList<ArrayList<Double>> alist = new ArrayList<ArrayList<Double>>() ; 
			...
			for(double i=00.0001;i<0.01;i+=.00001){
				System.out.println(i) ;
				
				int index = (int)(arr.size()*Math.random()*.5) ;			
				permute_trailstop p2 = new permute_trailstop() ; 
				p2.trailStop  = i ; 
				p2.tps[0] = tp ; 
				p2.stops[0] = -stop ; 			
				int icount=0;
				for(int j=index;j<index+24*60*100;j++){
			...
			SaveArray.saveArrayList(alist, "lol.txt");
