...
public class jpy_optimize{
	...
	public static void main(String[]args){
		...
		ArrayList<dukas> arr = parseDukas.parseOne("C:\\Users\\Acer\\Documents\\forex3\\","USDJPY_UTC_1 Min_Bid_2008.01.01_2015.03.12.csv") ;
		...
		NumberFormat formatter2 = new DecimalFormat("#0000");
			...
			for(int i=0;i<50;i++){
				...
				for(double tp=0.01;tp<1;tp+=.02){ scount++ ; alist.add(new ArrayList<Double>()) ;
					int tpcount = 0 ; 
					for(double stop=0.05;stop<2.20;stop+=0.05){ tpcount++ ; 
						int icount = 0 ; 
			...
			//SaveArray.saveArrayList(alist, "money_talks\\"+formatter2.format(scount)+"_"+formatter2.format(tpcount)+"_"+formatter.format(stop)+"_"+formatter.format(tp)+".txt");
				...
				SaveArray.saveArrayList(alist, "mts\\mt_USDJPY\\" + formatter2.format(i) +".txt");
			}
	}
}
